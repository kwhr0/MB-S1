...
Copyright(C) Common Source Code Project, Sasaji 2011-2021 All Rights Reserved.
==============================================================================
...
● 動作確認環境
  ...
  MacOSX版 : Mac OS X High Sierra (10.13.6) / El Capitan(10.11.6)
...
● 動作に必要なもの
  ...
  Agar版ではメニューやダイアログの漢字フォルダ名は文字化けして読めない場合
  があります。
  IPAフォント(ipagp.ttf)を置くことで日本語が表示できます。
  (UTF-8が表示できる環境であることが必要。)
  フォント名やフォントのあるパスは設定ダイアログにて設定できます。

  * 独立行政法人情報処理推進機構 (IPA) によって配布されているIPAフォント
...
● インストール
  ...
  2. 実機から取り出したROMイメージを同じフォルダまたは適当なフォルダに置いて
   ...
   ・漢字のROMイメージがない場合は、toolフォルダにあるkanji.exeを実行して、
     ...
     IPAフォントは同じフォルダまたはリソースフォルダに置いてください。
     設定ダイアログにてパスの指定もできます。
...
○ メニュー説明
  ...
  Agar版のメニューバーはマウスカーソルを描画ウインドウの上端近くに持って行くことで
  表示されます。
...
「画面(Screen)」メニュー
  ...
  【注意2】Mac Cocoa版ではフルスクリーンでメニューが表示されません。
...
○ キー操作説明
  ...
  ---------------- ------------------- ---------------- ---------------------
  [BREAK]          [F9]                [F9]             [F9]
  [GRAPH]          [F7] / [左command]  [F7] / [無変換]  [F7] / [無変換]
  [カタ/ひら]      [F8] / [右option]   [F8]             [F8]
  [変換]           [右command]         [変換]           [変換]
  [DEL]            [delete x]          [Delete]         [Delete]
  [INS]            [insert] / [fn]     [Insert]         [Insert]
  [BS]             [delete]            [Backspace]      [Backspace]
  [CAPSLOCK]       [F6] / [CAPS]       [F6] / [CAPS]    [F6] / [Shift]+[CAPS]
  [テンキー ?]     [end]               [END]            [END]
  [テンキー ,]     [page down]         [Page Down]      [Page Down]
  [COPY]           [F13]               [F12]            [F12]
  リセットスイッチ [F10]               [F11]            [F11]

  また、ショートカットキーとして以下があります。
  ※キー割り当てダイアログにて変更できます。

  機能             Mac側のキー         Linux側のキー    Windows側のキー 
  ---------------- ------------------- ---------------- ---------------------
  ポーズON/OFF     [F16]               [Pause]          [Pause]

  いくつかのメニューはAltキー(Macでは左optionキー)と併用して呼び出せます。
...
○ 設定ダイアログ(Configure...)
  ...
  ■モード(Mode)タブ
   ...
   ●I/Oポートアドレス(I/O Port Address)
    どの周辺機器を有効にするかを指定します。
     ...
     ◆拡張PSG(Ex PSG)           $FFE6 - $FFE7 ...
      拡張PSGカード(S1用)
      ・FM音源として使用するには「サウンド」タブにある「拡張PSGポートでFM音源を
        使用する。」にチェックを入れてください。
     ◆OS-9拡張カード            $FE11 ... 
      MMUセグメントレジスタを有効にします。
     ◆通信カード                $FF77 ... 
  ...
  ■サウンド(Sound)タブ
   ...
   ●FM音源カードのクロック(Clock of FM Synthesis Card)
     FM音源カード($FF1E,$FF1F)のクロックを設定します。

   ●FM音源の割り込み信号接続先(Connect interrupt signal of FM Synthesis to)
     FM音源のタイマー機能を使用する場合、割り込み信号を接続する必要があります。

   ●拡張PSGポートでFM音源を使用する。(Use FM Synthesis on Extended PSG port.)
     チェックを入れると拡張PSGポートがFM音源相当になります。
     ※拡張PSGポートのFM音源はクロック2,016,000Hz固定になります。
...
○ 音量設定ダイアログ(Volume...)
  ...
  ExPSG FM .......... 拡張PSGのFM部から出力される音量を設定します。
                      ※拡張PSGポートでFM音源を使用する設定にした場合に有効。
  ExPSG SSG ......... 拡張PSGのPSG/SSG部から出力される音量を設定します。
  OPN FM ............ FM音源カードのFM部から出力される音量を設定します。
