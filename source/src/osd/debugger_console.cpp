...
#define IOADDR_FORMAT "%02X"
#else
#define ADDR_FORMAT "%08X"
#define PHYS_ADDR_FORMAT "%08X"
...
DebuggerConsole::DebuggerConsole(debugger_thread_t *p)
{
	...
	for(int i = st; i <= ed; i++) {
		...
		dev->debugger->now_going = 0;
		dev->debugger->now_debugging = true;
...
int DebuggerConsole::GetAddressType(int num)
{
	...
	if (num == 1) {
		...
		if (true) {
#endif
			PrintError(_T("Invalid parameter of type."));
			Cr();
			return -2;
		}
	}
...
void DebuggerConsole::CommandDumpMemory(int num)
{
	...
	for(uint64_t addr = (start_addr & ~0x0f); addr <= end_addr; addr++) {
...
#ifdef _MBS1
			if (num == 1) {
				Printf(_T(PHYS_ADDR_FORMAT), addr & addr_mask);
			} else {
				Printf(_T(PHYS_ADDR_FORMAT)_T(" "), current_dev->mem->debug_latch_address(addr & addr_mask));
		...
		if(addr < start_addr) {
			...
			Printf(_T(" ")_T(DATA_BYTE_FORMAT), data);
...
void DebuggerConsole::CommandAccessDevice()
{
	...
	if(paramnum == 2) {
		...
		// edit
		if(!dp->vm->debug_write_reg(num, HexaToInt(params[2]), HexaToInt(params[3]))) {
...
{
	UsageCmdStr1(s, _T("RD"), _T("<device name> [<regno> <value>]"), _T("Show/Edit register(s) in specified device."));
	if (s) return;
...
///
/// @param[in] num : 1: using physical address
void DebuggerConsole::CommandSearch(int num)
{
	if(paramnum >= 2 && *params[1] == _T('?')) {
		Cr();
	...
	for(uint64_t addr = start_addr; addr <= end_addr; addr++) {
		bool found = true;
		for(int i = pos+2, j = 0; i < paramnum; i++, j++) {
			if(current_dev->mem->debug_read_data8(type, (addr + j) & addr_mask) != list[j]) {
				found = false;
				break;
			}
		}
		if(found) {
			...
			for(int i = pos+2, j = 0; i < paramnum; i++, j++) {
				Printf(_T(" ")_T(DATA_FORMAT), list[j]);
			}
			Cr();
		}
	}
...
void DebuggerConsole::UsageSearch(bool s, int num)
{
	switch(num) {
	case 0:
		UsageCmdStr1(s, _T("S"), _T("<start address> <end address> <list>"), _T("Search data(s)."));
		break;
	case 1:
		UsageCmdStr1(s, _T("SP"), _T("<type> <start address> <end address> <list>"), _T("Search data(s) using physical address."));
		break;
	}
	if (s) return;
...
void DebuggerConsole::CommandSetFileName()
{
	...
	if(paramnum >= 2) {
		UTILITY::tcscpy(current_dev->debugger->file_path, _MAX_PATH, params[1]);
	}
	Printf(_T("Current dump file name is \"%s\"."), current_dev->debugger->file_path);
	Cr();
}
...
void DebuggerConsole::CommandLoadBinaryFile(int num)
{
	...
	bool rc = false;
	if(UTILITY::check_file_extension(current_dev->debugger->file_path, _T(".hex"))) {
		// load from Intel hex format file
		uint32_t offset = 0;
		if(paramnum >= (pos + 1)) {
			offset = HexaToInt(params[pos]);
		}
		rc = LoadIntelHexFile(current_dev->debugger->file_path, offset, addr_mask, type);
	} else if(UTILITY::check_file_extensions(current_dev->debugger->file_path, _T(".s"), _T(".mot"), NULL)) {
		// load from Motorola S-Record format file
		uint32_t offset = 0;
		if(paramnum >= (pos + 1)) {
			offset = HexaToInt(params[pos]);
		}
		rc = LoadMotorolaSFile(current_dev->debugger->file_path, offset, addr_mask, type);
	} else {
		...
		rc = LoadBinaryRawFile(current_dev->debugger->file_path, start_addr, end_addr, addr_mask, type);
	}
	if (rc) {
	...
	Print(current_dev->debugger->file_path);
}
...
{
	FILEIO fio;
	bool rc = fio.Fopen(file_path, FILEIO_READ_BINARY);
	if(!rc) return rc;
...
{
	FILEIO fio;
	bool rc = fio.Fopen(file_path, FILEIO_READ_ASCII);
	if(!rc) return rc;
...
void DebuggerConsole::CommandSaveBinaryFile(int num)
{
	...
	if(UTILITY::check_file_extension(current_dev->debugger->file_path, _T(".hex"))) {
		...
		rc = SaveIntelHexFile(current_dev->debugger->file_path, start_addr, end_addr, addr_mask, type);
	} else if(UTILITY::check_file_extensions(current_dev->debugger->file_path, _T(".s"), _T(".mot"), NULL)) {
		// save to Motorola S-Record format file
		rc = SaveMotorolaSFile(current_dev->debugger->file_path, start_addr, end_addr, addr_mask, type);
	} else {
		// save to binary dump file
		rc = SaveBinaryRawFile(current_dev->debugger->file_path, start_addr, end_addr, addr_mask, type);
	}
	if (rc) {
...
{
	FILEIO fio;
	bool rc = fio.Fopen(file_path, FILEIO_WRITE_BINARY);
	if(!rc) return rc;
...
{
	FILEIO fio;
	bool rc = fio.Fopen(file_path, FILEIO_WRITE_ASCII);
	if(!rc) return rc;
...
void DebuggerConsole::UsageSetBreakPoint(bool s, int num)
{
	...
	case BP_INTR:
		...
		Printf(_T("  <signal name> - %s"), buffer);
		Cr();
...
void DebuggerConsole::CommandExecute()
{
	...
	while (!dp->request_terminate && !breaked && !NowBreakPointInDebugger()) {
		...
		while(!dp->request_terminate && !current_dev->debugger->now_suspended) {
			if (EscapePressed()) {
				breaked |= 1;
				break;
			} else if (NowDisable()) {
		...
		while(!dp->request_terminate && !current_dev->debugger->now_suspended && !NowDisable()) {
			CDelay(10);
		}
...
//		int idx = bps->HitIndex();
		...
		case BreakPoints::BP_READ_MEMORY_PH:
			UTILITY::stprintf(buffer, DC_MAX_BUFFER_LEN, _T("Breaked at %s on %s : memory %s was read at %s"),
				_T(ADDR_FORMAT), storage->GetCurrentCPUName(), _T(ADDR_FORMAT), _T(ADDR_FORMAT));
			Printf(buffer, current_dev->cpu->get_next_pc(), bp->Addr(), current_dev->cpu->get_pc());
			break;
		case BreakPoints::BP_WRITE_MEMORY:
		case BreakPoints::BP_WRITE_MEMORY_PH:
			UTILITY::stprintf(buffer, DC_MAX_BUFFER_LEN, _T("Breaked at %s on %s : memory %s was written at %s"),
				_T(ADDR_FORMAT), storage->GetCurrentCPUName(), _T(ADDR_FORMAT), _T(ADDR_FORMAT));
			Printf(buffer, current_dev->cpu->get_next_pc(), bp->Addr(), current_dev->cpu->get_pc());
			break;
		case BreakPoints::BP_INPUT_IO:
			UTILITY::stprintf(buffer, DC_MAX_BUFFER_LEN, _T("Breaked at %s on %s : port %s was read at %s"),
				_T(ADDR_FORMAT), storage->GetCurrentCPUName(), _T(ADDR_FORMAT), _T(ADDR_FORMAT));
			Printf(buffer, current_dev->cpu->get_next_pc(),bp->Addr(), current_dev->cpu->get_pc());
			break;
		case BreakPoints::BP_OUTPUT_IO:
			UTILITY::stprintf(buffer, DC_MAX_BUFFER_LEN, _T("Breaked at %s on %s : port %s was written at %s"),
				_T(ADDR_FORMAT), storage->GetCurrentCPUName(), _T(IOADDR_FORMAT), _T(ADDR_FORMAT));
			Printf(buffer, current_dev->cpu->get_next_pc(), bp->Addr(), current_dev->cpu->get_pc());
			break;
		case BreakPoints::BP_INTERRUPT: {
			...
			UTILITY::stprintf(buffer, DC_MAX_BUFFER_LEN, _T("Breaked at %s on %s : %s %s"),
...
void DebuggerConsole::CommandTraceBack()
{
	...
	for(int i=steps; i>=0; ) {
		if (current_dev->cpu->debug_trace_back(i, buffer, DC_MAX_BUFFER_LEN) >= -1) {
			Print(Yellow, _T("  "), false);
			Out();
		}
		if ((i = current_dev->cpu->debug_trace_back_regs(i, buffer, DC_MAX_BUFFER_LEN)) >= -1) {
			Out(White);
		}
	}
	SetTextColor(White);
...
void DebuggerConsole::CommandOutputLogFile()
{
	...
	if(paramnum >= 2) {
		...
		if (_tcsicmp(params[1], _T("CLOSE")) != 0) {
			...
			logfile = new FILEIO();
			if (!logfile->Fopen(logfilename, FILEIO_WRITE_ASCII)) {
				delete logfile;
				logfile = NULL;
				logfilename[0] = _T('\0');
			}
...
void DebuggerConsole::CommandSendKeyToEmulator()
{
	if(paramnum == 3 || paramnum == 4) {
		...
		key_stat_tmp[code] = MAX((int)(p->vm->frame_rate() * (double)msec / 1000.0 + 0.5), 1);
#else
...
//					break;
	...
	try {
		for(int i=0; i<DC_MAX_CPUS; i++) {
			...
			if (dev->debugger) {
				dev->debugger->now_going = 0;
				dev->debugger->now_debugging = dev->debugger->now_suspended = false;
			}
		}
	} catch(...) {
	}
