...
{
#ifdef USE_AUTO_KEY
	if(autokey_phase == 0) return;
	if(autokey_phase == USE_AUTO_KEY && autokey_code) {
	...
	if(autokey_phase == USE_AUTO_KEY + 3 && key_code == AUTO_KEY_NONE) {
		logging->out_debugf(_T("recognized_key: phase:%d auto_key:%x vm_key:%x"),autokey_phase,autokey_code,key_code);
...
bool EMU::start_auto_key(const char *str)
{
	...
	int prev_code = 0;
	int prev_kana = 0;
	parse_auto_key(str, size, prev_code, prev_kana);
	if(prev_kana) {
		autokey_buffer->write(AUTO_KEY_KANA);
#ifdef USE_EMU_INHERENT_SPEC
		if (prev_kana == 0x1000) autokey_buffer->write(AUTO_KEY_KANA);
#endif
	}
...
bool EMU::open_auto_key(const _TCHAR *file_path)
{
	...
	fp = new FILEIO();
	int prev_kana = 0;
	if (fp->Fopen(file_path, FILEIO_READ_BINARY)) {
		autokey_buffer->clear();
		int prev_code = 0;
		int size = 0;
		while((size = (int)fp->Fread(buf, sizeof(char), 1024)) > 0) {
			parse_auto_key(buf, size, prev_code, prev_kana);
		}
		fp->Fclose();
...
void EMU::parse_auto_key(const char *buf, int size, int &prev_code, int &prev_kana)
{
...
#ifdef USE_EMU_INHERENT_SPEC
			int kana = code & 0x3000;
			if(kana - prev_kana == 0x1000) {
				autokey_buffer->write(AUTO_KEY_KANA);
			} else if(kana - prev_kana == 0x2000) {
				autokey_buffer->write(AUTO_KEY_KANA);
				autokey_buffer->write(AUTO_KEY_KANA);
			}
			if(prev_kana - kana == 0x2000) {
				autokey_buffer->write(AUTO_KEY_KANA);
			} else if(prev_kana - kana == 0x1000) {
				autokey_buffer->write(AUTO_KEY_KANA);
				autokey_buffer->write(AUTO_KEY_KANA);
			}
#else
