...
#ifdef USE_FD1
bool EMU::open_disk_main(int drv, const _TCHAR* file_path, int offset, uint32_t flags)
{
	if(disk_inserted(drv) || disk_insert[drv].wait_count != 0) {
		...
		return vm->open_disk(drv, file_path, offset, flags);
#else
			// wait 0.5sec
#ifdef SUPPORT_VARIABLE_TIMING
...
bool EMU::open_disk(int drv, const _TCHAR* file_path, int bank_num, int offset, uint32_t flags)
{
	...
	if (vm && file_path && file_path[0] != 0) {
		if (open_disk_main(drv, file_path, offset, flags)) {
...
//	memset(&d88_file[drv], 0, sizeof(d88_file_t));
	d88_files.GetFile(drv).Clear();
//	d88_file[drv].bank_nums = 0;
//	d88_file[drv].cur_bank = -1;
//	d88_file[drv].prev_bank = -2;
}
...
{
	if (ledbox) {
		config.show_dlg ^= MSK_SHOWLEDBOX;
...
{
	if (ledbox) {
		config.show_dlg ^= MSK_INSIDELEDBOX;
		ledbox->Show(FLG_LEDBOX_ALL);
	}
}
...
#ifdef USE_MESSAGE_BOARD
	if (msgboard) {
		config.show_dlg ^= MSK_SHOWMSGBOARD;
...
void EMU::change_use_joypad(int num)
{
	...
	switch(num) {
	case 1:
		config.show_dlg = ((config.show_dlg & ~MSK_USEJOYSTICK_ALL) | MSK_USEJOYSTICK);
		use_joystick = true;
		reset_joystick();
		break;
	case 2:
		config.show_dlg = ((config.show_dlg & ~MSK_USEJOYSTICK_ALL) | MSK_USEPIAJOYSTICK);
		use_joystick = true;
		reset_joystick();
		break;
	default:
		config.show_dlg = (config.show_dlg & ~MSK_USEJOYSTICK_ALL);
		release_joystick();
		use_joystick = false;
		break;
	}
