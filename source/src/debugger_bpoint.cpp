...
void DEBUGGER_BPOINTS::find_fetch_break_trace_points(BreakPoints::en_break_point_type bp_type, uint32_t addr, int length, bool &suspended)
{
	if (bps[bp_type].FindFetch(addr, length)) {
		suspended = m_now_breakpoint = true;
	} else if (tps[bp_type].FindFetch(addr, length)) {
		suspended = m_now_tracepoint = true;
	}
}

void DEBUGGER_BPOINTS::find_mem_break_trace_points(BreakPoints::en_break_point_type bp_type, uint32_t addr, int length, uint32_t value, bool &suspended)
{
...
void DEBUGGER_BPOINTS::find_io_break_trace_points(BreakPoints::en_break_point_type bp_type, uint32_t addr, int length, uint32_t value,  bool &suspended)
{
...
void DEBUGGER_BPOINTS::find_intr_break_trace_points(BreakPoints::en_break_point_type bp_type, uint32_t addr, uint32_t mask, bool &suspended)
{
...
void DEBUGGER_BPOINTS::find_basic_break_trace_points(BreakPoints::en_break_point_type bp_type, DEVICE *d_mem, uint32_t addr, uint32_t data, bool &suspended)
{
	if (bps[bp_type].FindBASIC(d_mem, addr)) {
		suspended = m_now_basicreason = m_now_breakpoint = true;
	} else if (tps[bp_type].FindBASIC(d_mem, addr)) {
		suspended = m_now_basicreason = m_now_tracepoint = true;
	}
}
