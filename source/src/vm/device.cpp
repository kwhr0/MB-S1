...
void DEVICE::load_state(FILEIO* fio) {}
...
bool DEVICE::debug_ioport_is_supported()
{
	return true;
}
...
#ifdef USE_EMU_INHERENT_SPEC
// display
void DEVICE::update_vram(int v, int clock) {}
#endif
//
...
bool DEVICE::find_state_chunk(FILEIO *fio, const char *class_name, const char *identifier, vm_state_ident_t *vm_state_i)
{
	...
	for(int retry = 0; retry < 2; retry++) {
		while (fio->Fread(vm_state_i, size, 1) == 1) {
			...
			if (chunk_size > size) {
				fio->Fseek(chunk_size - size, FILEIO_SEEK_CUR);
			}
		}
		// retry first
		fio->Fseek(sizeof(vm_state_header_t), FILEIO_SEEK_SET);
	}
	return false;
}
