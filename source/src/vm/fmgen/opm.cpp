...
void OPM::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(OPM_STATE_VERSION);	// 4 bytes
	...
	state_fio->FputUint32_LE(pcmrate);	// 4 bytes
	state_fio->FputUint32_LE(pmd);	// 4 bytes
	state_fio->FputUint32_LE(amd);	// 4 bytes

	state_fio->FputUint32_LE(lfocount);	// 4 bytes
	state_fio->FputUint32_LE(lfodcount);	// 4 bytes
	state_fio->FputUint32_LE(lfo_count_);	// 4 bytes
	state_fio->FputUint32_LE(lfo_count_diff_);	// 4 bytes

	state_fio->FputUint32_LE(lfo_step_);	// 4 bytes
	state_fio->FputUint32_LE(lfo_count_prev_);	// 4 bytes
	state_fio->FputUint32_LE(lfowaveform);	// 4 bytes
	state_fio->FputUint32_LE(rateratio);	// 4 bytes

	state_fio->FputUint32_LE(noise);	// 4 bytes
	state_fio->FputInt32_LE(noisecount);	// 4 bytes
	state_fio->FputUint32_LE(noisedelta);	// 4 bytes
	state_fio->FputBool(interpolation);	// 1 byte
	state_fio->FputUint8(lfofreq);	// 1 byte
	state_fio->FputUint8(status);	// 1 byte
	state_fio->FputBool(interrupt);	// 1 byte

	state_fio->FputUint8(reg01);	// 1 byte
	state_fio->FputUint8(0);	// 1 byte dummy
	state_fio->FputUint8(0);	// 1 byte dummy
	state_fio->FputUint8(0);	// 1 byte dummy

	state_fio->Fwrite(kc, sizeof(kc), 1);	// 8 bytes
	state_fio->Fwrite(kf, sizeof(kf), 1);	// 8 bytes
	state_fio->Fwrite(pan, sizeof(pan), 1);	// 8 bytes
	...
	if (size) *size += 112;	// 16 bytes bound
}
...
bool OPM::LoadState(void *f)
{
	...
	fmvolume_l = state_fio->FgetInt32_LE();
#ifdef USE_FMGEN_STEREO
	fmvolume_r = state_fio->FgetInt32_LE();
#else
