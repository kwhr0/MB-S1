...
void OPNBase::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(OPN_BASE_STATE_VERSION);	// 4 bytes

	state_fio->FputInt32_LE(fmvolume_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(fmvolume_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(fmvolume_l);	// 4 bytes
#endif
	state_fio->FputUint32_LE(clock);	// 4 bytes

	state_fio->FputUint32_LE(rate);	// 4 bytes
	state_fio->FputUint32_LE(psgrate);	// 4 bytes
	state_fio->FputUint32_LE(status);	// 4 bytes
	state_fio->FputBool(interrupt);	// 1 byte
	state_fio->FputUint8(prescale);	// 1 byte
	state_fio->FputInt8(0);	// 1 byte dummy
	state_fio->FputInt8(0);	// 1 byte dummy
	...
	if (size) *size += 32;	// 16 bytes bound
}
...
void OPN::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(OPN_STATE_VERSION);	// 4 bytes

	for(int i = 0; i < 3; i++) {		// 12 bytes
		state_fio->FputUint32_LE(fnum[i]);
	}
	for(int i = 0; i < 3; i++) {		// 12 bytes
		state_fio->FputUint32_LE(fnum3[i]);
	}
	for(int i = 0; i < 6; i++) {		// 6 bytes
		state_fio->FputUint8(fnum2[i]);
	}
	for(int i = 0; i < 14; i++) {		// 14 bytes dummy
		state_fio->FputInt8(0);
	}
	for(int i = 0; i < 3; i++) {
	...
	if (size) *size += 48;	// 16 bytes bound
}
...
void OPNABase::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(OPNA_BASE_STATE_VERSION);	// 4 bytes

	OPNBase::SaveState(f, size);
	state_fio->Fwrite(pan, sizeof(pan), 1);	// 6 bytes
	state_fio->Fwrite(fnum2, sizeof(fnum2), 1);	// 9 bytes
	state_fio->FputUint8(reg22);	// 1 byte

	state_fio->FputUint32_LE(reg29);	// 4 bytes
	state_fio->FputUint32_LE(stmask);	// 4 bytes
	state_fio->FputUint32_LE(statusnext);	// 4 bytes

	state_fio->FputUint32_LE(lfocount);	// 4 bytes
	state_fio->FputUint32_LE(lfodcount);	// 4 bytes
	...
	state_fio->FputUint32_LE(adpcmmask);	// 4 bytes

	state_fio->Fwrite(adpcmbuf, 0x40000, 1);	// 262144 bytes

	state_fio->FputUint32_LE(adpcmnotice);	// 4 bytes
	state_fio->FputUint32_LE(startaddr);	// 4 bytes
	state_fio->FputUint32_LE(stopaddr);	// 4 bytes
	state_fio->FputUint32_LE(memaddr);	// 4 bytes

	state_fio->FputUint32_LE(limitaddr);	// 4 bytes
	state_fio->FputInt32_LE(adpcmlevel);	// 4 bytes
	state_fio->FputInt32_LE(adpcmvolume_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(adpcmvolume_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(adpcmvolume_l);	// 4 bytes
#endif
	state_fio->FputInt32_LE(adpcmvol_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(adpcmvol_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(adpcmvol_l);	// 4 bytes
#endif
	state_fio->FputUint32_LE(deltan);	// 4 bytes
	state_fio->FputInt32_LE(adplc);	// 4 bytes

	state_fio->FputInt32_LE(adpld);	// 4 bytes
	state_fio->FputUint32_LE(adplbase);	// 4 bytes
	state_fio->FputInt32_LE(adpcmx);	// 4 bytes
	state_fio->FputInt32_LE(adpcmd);	// 4 bytes

	state_fio->FputInt32_LE(adpcmout_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(adpcmout_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(adpcmout_l);	// 4 bytes
#endif
	state_fio->FputInt32_LE(apout0_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(apout0_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(apout0_l);	// 4 bytes
#endif

	state_fio->FputInt32_LE(apout1_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(apout1_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(apout1_l);	// 4 bytes
#endif
	state_fio->FputUint32_LE(adpcmreadbuf);	// 4 bytes

	state_fio->FputBool(adpcmplay);	// 1 byte
	state_fio->FputInt8(granuality);	// 1 byte
	state_fio->FputBool(adpcmmask_);	// 1 byte
	state_fio->FputUint8(control1);	// 1 byte

	state_fio->FputUint8(control2);	// 1 byte
	state_fio->FputInt8(0);	// 1 byte dummy
	state_fio->FputInt8(0);	// 1 byte dummy
	state_fio->FputInt8(0);	// 1 byte dummy

	state_fio->Fwrite(adpcmreg, sizeof(adpcmreg), 1);	// 8 bytes

	state_fio->FputInt32_LE(rhythmmask_);	// 4 bytes

	for(int i = 0; i < 6; i++) {
		ch[i].SaveState(f, size);
	}

	if (size) *size += 262144 + 144;
}
...
//	初期化
//
bool OPNA::Init(uint32_t c, uint32_t r, bool ipflag, const _TCHAR* path)
{
	rate = 8000;
...
void OPNA::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(OPNA_STATE_VERSION);	// 4 bytes

	OPNABase::SaveState(f, size);
	for(int i = 0; i < 6; i++) {	// 6*6 = 36 bytes
		state_fio->FputUint8(rhythm[i].pan);
		state_fio->FputInt8(rhythm[i].level);
		state_fio->FputUint32_LE(rhythm[i].pos);
	}
	state_fio->FputInt8(rhythmtl);	// 1 byte
	state_fio->FputUint8(rhythmkey);	// 1 byte
	state_fio->FputInt8(0);	// 1 byte dummy
	state_fio->FputInt8(0);	// 1 byte dummy

	state_fio->FputInt32_LE(rhythmtvol_l);	// 4 bytes
#ifdef USE_FMGEN_STEREO
	state_fio->FputInt32_LE(rhythmtvol_r);	// 4 bytes
#else
	state_fio->FputInt32_LE(rhythmtvol_l);	// 4 bytes
#endif

	state_fio->FputInt32_LE(0);	// 4 bytes dummy
	state_fio->FputInt32_LE(0);	// 4 bytes dummy
	state_fio->FputInt32_LE(0);	// 4 bytes dummy

	if (size) *size += 64;	// 16 bytes bound
}
