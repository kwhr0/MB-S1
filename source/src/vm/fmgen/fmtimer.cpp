...
void Timer::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(TIMER_STATE_VERSION);	// 4 bytes

	state_fio->FputUint8(status);	// 1 byte
	state_fio->FputUint8(regtc);	// 1 byte
	state_fio->Fwrite(regta, sizeof(regta), 1);	// 2 bytes
	state_fio->FputInt32_LE(timera);	// 4 bytes
	state_fio->FputInt32_LE(timera_count);	// 4 bytes

	state_fio->FputInt32_LE(timerb);	// 4 bytes
	state_fio->FputInt32_LE(timerb_count);	// 4 bytes
	state_fio->FputInt32_LE(prescaler);	// 4 bytes
