...
void PSG::SaveState(void *f, size_t *size)
{
	...
	state_fio->FputUint32_LE(PSG_STATE_VERSION);	// 4 bytes

	state_fio->Fwrite(reg, sizeof(reg), 1);	// 16 bytes
	state_fio->FputInt32_LE((int)(envelop_l - &enveloptable_l[0][0]));	// 4 bytes
	for (int i = 0; i < 3; i++) {	// 12 bytes
		state_fio->FputUint32_LE(olevel_l[i]);
	}
	for (int i = 0; i < 3; i++) {	// 12 bytes
#ifdef USE_FMGEN_STEREO
		state_fio->FputUint32_LE(olevel_r[i]);
#else
		state_fio->FputUint32_LE(olevel_l[i]);
#endif
	}
	for (int i = 0; i < 3; i++) {	// 12 bytes
		state_fio->FputUint32_LE(scount[i]);
	}
	for (int i = 0; i < 3; i++) {	// 12 bytes
		state_fio->FputUint32_LE(speriod[i]);
	}
	state_fio->FputUint32_LE(ecount);	// 4 bytes
	state_fio->FputUint32_LE(eperiod);	// 4 bytes
	state_fio->FputUint32_LE(ncount);	// 4 bytes
	state_fio->FputUint32_LE(nperiod);	// 4 bytes
	state_fio->FputUint32_LE(tperiodbase);	// 4 bytes
	state_fio->FputUint32_LE(eperiodbase);	// 4 bytes
	state_fio->FputUint32_LE(nperiodbase);	// 4 bytes
	state_fio->FputInt32_LE(mask);	// 4 bytes
