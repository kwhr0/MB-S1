...
// interrupt status
#define FDC_STI_STRB		0x08	// STRB
#define FDC_STI_STSREQ		0x04	// status sense request
#define FDC_STI_SETCOMP		0x02	// setting time complete
#define FDC_STI_CMDCOMP		0x01	// macro command complete

// status code A
#define FDC_STA_BUSY		0x80	// busy
#define FDC_STA_INDEX		0x40	// index hole
#define FDC_STA_TRACKNE		0x20	// track not equal
#define FDC_STA_WRITEP		0x10	// write protect
#define FDC_STA_TRACK00		0x08	// track zero
#define FDC_STA_DREADY		0x04	// drive ready
#define FDC_STA_DELETE		0x02	// delete data mark detected
#define FDC_STA_DRQ			0x01	// data transfar request

// status code B
#define FDC_STB_HARDERR		0x80	// hard error
#define FDC_STB_WRITEERR	0x40	// write error
#define FDC_STB_FILEINO		0x20	// file inoperable
#define FDC_STB_SEEKERR		0x10	// seek error
#define FDC_STB_SECTNF		0x08	// sector address undeteced
#define FDC_STB_DATANF		0x04	// data mark undetected
#define FDC_STB_CRCERR		0x02	// crc error
#define FDC_STB_DATAERR		0x01	// data transfar error

// command register
#define FDC_CMR_FWF			0x10	// free format write flag
#define FDC_CMR_DMA			0x20	// DMA flag
#define FDC_CMR_ISR3MASK	0x40	// ISR3 interrupt mask
#define FDC_CMR_FUNCMASK	0x80	// Function interrupt mask

// macro commands
#define FDC_CMD_FFW_END 0x00	// stop of multi sector write
#define FDC_CMD_STZ	0x02	// seek track zero
#define FDC_CMD_SEK	0x03	// seek
#define FDC_CMD_SSR	0x04	// single sector read
#define FDC_CMD_SSW	0x05	// single sector write
#define FDC_CMD_RCR	0x06	// read CRC
#define FDC_CMD_SWD	0x07	// single sector write with delete data mark
#define FDC_CMD_FFR	0x0a	// free format read
#define FDC_CMD_FFW	0x0b	// free format write
#define FDC_CMD_MSR	0x0c	// multi sector read
#define FDC_CMD_MSW	0x0d	// multi sector write

#define EVENT_SEEK		0
#define EVENT_SEARCH	1
#define EVENT_MULTI		2
#define EVENT_LOST		3
#define EVENT_DRQ		4
...
void MC6843::load_state(FILEIO *fp)
{
...
{
	buffer[0] = _T('\0');
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 0, _T("CTAR"), ctar);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 1, _T("CMR"), cmr);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 2, _T("ISR"), isr);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 3, _T("SUR"), sur);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 4, _T("STRA"), stra);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 5, _T("STRB"), strb);
	UTILITY::tcscat(buffer, buffer_len, _T("\n"));
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 6, _T("SAR"), sar);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 7, _T("GCR"), gcr);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 8, _T("CCR"), ccr);
	UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 9, _T("LTAR"), ltar);
}
#endif
