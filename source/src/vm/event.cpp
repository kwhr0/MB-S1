...
void EVENT::drive(int split_num)
{
	...
	for(int epow = epowstart; epow > 0; epow--) {
		...
		for(int i=0; i<ncount_cpu; i++) {
			now_suspend |= d_cpu[i].device->now_suspend();
		}
		if (!now_suspend) {
			vsuspend = -1;
			epowsuspend = -1;
			...
			for(v = vstart; v < vline_split[frame_split_num+1]; v++) {
...
#ifdef USE_DEBUGGER
									now_suspend |= (sub_clock > 0 ? d_cpu[i].device->reach_break_point() : d_cpu[i].device->now_suspend());
#endif /* USE_DEBUGGER */
								}
							}
#ifdef USE_DEBUGGER
				...
				if (event_remain <= 0) {
					...
					for(int i = 0; i < dcount_display; i++) {
						d_display[i]->update_vram(v, vclocks[v]);
					}
#endif
					update_sound(v);
				}
...
void EVENT::load_state(FILEIO *fio)
{
	...
	for(int i=0; i<MAX_EVENT; i++) {
		...
		fio->Fseek(sizeof(vm_state->event[i].reserved), FILEIO_SEEK_CUR);
	...
	if (Uint16_LE(vm_state_i.version) >= 0x42) {
		...
		fio->Fseek(sizeof(vm_state->reserved), FILEIO_SEEK_CUR);
		...
		ncount_cpu = fio->FgetUint8();
		fio->Fseek(sizeof(vm_state->reserved), FILEIO_SEEK_CUR);
		fio->Fseek(sizeof(vm_state->event_remain), FILEIO_SEEK_CUR);
		fio->Fseek(sizeof(vm_state->cpu_remain), FILEIO_SEEK_CUR);
		fio->Fseek(sizeof(vm_state->cpu_accum), FILEIO_SEEK_CUR);
	} else {
		// L3 compatible
		event_clocks <<= 1;
		if (Uint16_LE(vm_state_i.version) >= 2) {
			fio->Fseek(sizeof(vm_state->cpu_speed_half), FILEIO_SEEK_CUR);
			ncount_cpu = fio->FgetUint8();
			fio->Fseek(sizeof(vm_state->reserved), FILEIO_SEEK_CUR);
			event_remain = fio->FgetInt32_LE();
			cpu_remain = fio->FgetInt32_LE();
