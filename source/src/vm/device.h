...
#define SIG_CPU_BABS	196
#define SIG_CPU_ADD_DEAD_CYCLE	197
#define SIG_CPU_HALT	198
#define SIG_CPU_RESET	199
#endif
...
//	virtual void set_volume(int ch, int decibel_l, int decibel_r);
	...
	// network and communication device
	...
	virtual bool debug_ioport_is_supported();
...
#ifdef USE_EMU_INHERENT_SPEC
	// display
	virtual void update_vram(int v, int clock);
#endif
	//
...
#define FIND_STATE_CHUNK(p_fio, st_i) \
	{ \
		...
		p_fio->Fseek(-(long)sizeof(st_i), FILEIO_SEEK_CUR); \
	}
...
#define READ_STATE_CHUNK(p_fio, st_i, st_v) \
	{ \
		...
		if (i_size >= (sizeof(st_v) + sizeof(st_i))) { \
			...
			p_fio->Fseek(i_size - sizeof(st_v) - sizeof(st_i), FILEIO_SEEK_CUR); \
		} else { \
