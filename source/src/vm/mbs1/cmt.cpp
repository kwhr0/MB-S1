...
void CMT::set_data(uint32_t data)
{
	...
	if (REG_REMOTE && recv_ok && rec) {
		...
		if (data_write_pos >= CMTDATA_BUFFER_SIZE) {
			...
			if (d_ctrl != NULL) {
				d_ctrl->write_signal(SIG_ACIA_ERROR, 1, 1);
			}
		} else {
			switch (file_type) {
			case 0:
...
void CMT::set_baud(int sel)
{
	...
	// double speed
	int spd = ((sel & 4) >> 2);
	tape_wav_freq = (TAPE_WAV_FREQ << spd);
	baud_rate <<= spd;
...
bool CMT::rec_datarec(const _TCHAR* filename)
{
	...
	if(!(fio->Fopen(filename, FILEIO_READ_WRITE_BINARY)
	  || fio->Fopen(filename, FILEIO_WRITE_BINARY))) {
		// open failed
		return false;
	}
	fio->Fclose();
...
int CMT::load_image(int dir)
{
	...
	if (dir >= 0) {
		...
		if (nowpos <= file_startpos) {
			fio->Fseek(file_startpos, FILEIO_SEEK_SET);
			data_read_size = 0;
		} else {
			if (fio->Fseek(dir, FILEIO_SEEK_CUR) != 0) {
				data_read_size = 0;
			} else {
				data_read_size = (int)fio->Fread(data_buffer, sizeof(uint8_t), 1 /*sizeof(data_buffer)*/);
				fio->Fseek(dir, FILEIO_SEEK_CUR);
			}
		}
	}
	data_read_pos = 0;
	...
	case 0:
		if(REG_REMOTE) {
			...
			if (d_ctrl != NULL) {
				// send to rxclk
				send_ok = false;
				d_ctrl->write_signal(SIG_ACIA_RXCLK, 1, 1);
				...
				if (play && (config.realmode_datarec || send_ok || now_reset)) {
					...
					// send data to acia
					d_ctrl->write_signal(SIG_ACIA_RXDATA, data, mask);
				}
			}
			// receive from acia
			...
			if (d_ctrl != NULL) {
				...
				txdata_received = false;
				txdata = 0xff;
				d_ctrl->write_signal(SIG_ACIA_TXCLK, 1, mask);
			}
			if (rec) {
				// set data to buffer
...
void CMT::mix(int32_t* buffer, int cnt)
{
	...
	// relay wave sound
	if ((relay_wav_enable & 1) != 0 && REG_REMOTE != 0 && remote_prev == 0) {
		// relay on sound
		relay_play = 1;
		relay_play_pos = 0;
	} else if ((relay_wav_enable & 2) != 0 && REG_REMOTE == 0 && remote_prev != 0) {
		// relay off sound
		relay_play = 2;
		relay_play_pos = 0;
	}
...
{
	relay_volume = int(16384.0 * pow(10.0, decibel_relay / 40.0));
	if (mute_relay) relay_volume = 0;

	tape_volume = int(16384.0 * pow(10.0, decibel_tape / 40.0));
