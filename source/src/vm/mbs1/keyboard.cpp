...
#ifdef _MBS1
	// key counter clock 128 = CPU_CLOCKS / 262line / 60fps
	register_event_by_clock(this, SIG_KEYBOARD_HSYNC, CPU_CLOCKS / 262 / 60, true, &hsync_register_id);
#endif
}
...
#ifdef _MBS1
	if (hsync_register_id >= 0) {
		cancel_event(this, hsync_register_id);
	}
	// key counter clock 128 = CPU_CLOCKS / 262line / 60fps
	register_event_by_clock(this, SIG_KEYBOARD_HSYNC, CPU_CLOCKS / 262 / 60, true, &hsync_register_id);
#endif
...
//				d_pia->write_signal(SIG_PIA_PB, joy_pia[0], 0xff);
//			}
			break;
#else
		case SIG_KEYBOARD_PIA_PA:
			...
			d_pia->write_signal(SIG_PIA_PA, ~joy_pia[joy_pia_sel], 0x3f);	// negative
			break;
#endif
#endif
		case SIG_CPU_RESET:
...
#ifdef _MBS1
		pressed = (IOPORT_USE_KEYBD && pressing_key(scan_code));
#endif
		if (pressed == true) {
			scan_code |= 0x80;
			key_pressed = 2;
...
#ifdef _MBS1
					emu->change_archtecture(VM::ArchSysMode, -1, false);
#endif
				}
...
{
	if (lpen_bl && FLG_USELIGHTPEN) {
#ifdef USE_KEY_RECORD
		reckey->recording_lightpen_status();
#endif
		lpen_flg = (mouse_stat[2] & 1) ? 0x80 : 0;
		if (lpen_flg != 0) {
			// push light pen
			// LPSTB signal
			d_disp->write_signal(SIG_DISPLAY_LPSTB
		...
		for(int n = 0; sjoy2joy_button_map[n] >= 0; n++) {
			for(int i=0; i<2; i++) {
				...
				if (joy_stat[i] & code) {
					...
					// irq
					d_pia->write_signal(i == 0 ? SIG_PIA_CA1 : SIG_PIA_CA2,0,1);
				} else {
					d_pia->write_signal(i == 0 ? SIG_PIA_CA1 : SIG_PIA_CA2,1,1);
				}
			}
		}
		d_pia->write_signal(SIG_PIA_PA, ~joy_pia[joy_pia_sel], 0x3f);	// negative
...
#ifdef USE_KEY_RECORD
	reckey->recording_joypia_status(joy_pia);
#endif

#ifdef USE_PIAJOYSTICKBIT
	if (FLG_USEPIAJOYSTICK) {
		d_pia->write_signal(SIG_PIA_PB, sjoy2joy_map[0x17][0] ? ~joy_pia[0] : joy_pia[0], 0xff);
	}
#endif
#endif
}
...
#ifdef USE_KEY_RECORD
	pressed = reckey->recording_keys(key_code, pressed);
#endif
...
{
#ifdef USE_KEY_RECORD
	reckey->recording_system_keys(code, 1);
#endif
}
...
{
#ifdef USE_KEY_RECORD
	reckey->recording_system_keys(code, 0);
#endif
}
...
void KEYBOARD::load_state(FILEIO *fio)
{
	...
	if (vm_state_i.version >= 2) {
		config.show_dlg = (vm_state.lpen_bl & 2) ? (config.show_dlg | MSK_USELIGHTPEN) : (config.show_dlg & ~MSK_USELIGHTPEN);
	}
	key_scan_code = 0;
#ifdef USE_PIAJOYSTICK
...
#endif
	int id = is_registerd_event(this_class_name, this_identifier, -1);
	if (id >= 0) {
		// already registerd
		hsync_register_id = id;
	} else {
		// regist
		// key counter clock 128 = CPU_CLOCKS / 262line / 60fps
		register_event_by_clock(this, SIG_KEYBOARD_HSYNC, CPU_CLOCKS / 262 / 60, true, &hsync_register_id);
