...
#if !defined(DEBUG_READ_OK) && defined(USE_Z80B_CARD)
...
//			d_disp->WRITE_IO8(addr, data);
		...
		case 0xeffd0:
			...
			d_disp->WRITE_IO8(addr, data);
			d_comm->write_signal(SIG_COMM_RS, data, 0x20);
			break;
		case 0xeffd1:
			// trace counter
#ifdef WRITE_SIGNAL
		...
		case 0xeffd2:
			// remote switch
			d_cmt->write_signal(SIG_CMT_REMOTE, data, 0x80);
			break;
		case 0xeffd3:
			// music sel
//			logging->out_debugf("ms %02x",data);
			d_sound->write_signal(SIG_SOUND_ON, 1, 1);
			d_sound->write_signal(SIG_SOUND_SIGNAL, data, 0x80);
			break;
		case 0xeffd4:
			// time mask
...
//			logging->out_debugf(_T("lpengl: %02x"),data);
		...
		case 0xeffd7:
			// baud sel
			d_cmt->write_signal(SIG_CMT_BAUD_SEL, data, 0x0f);
			break;
		case 0xeffd8:
			// color register
#ifdef _DEBUG_CRAM
...
//				logging->out_debugf("psg2w dat:%02x",data);
		...
		case 0xeffe7:
			// ex psg reg
			if (IOPORT_USE_EXPSG) {
				d_psgex->WRITE_IO8(0, data & (config.use_opn_expsg ? 0xff :0x0f));
