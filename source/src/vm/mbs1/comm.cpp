...
//			logging->out_logf(LOG_DEBUG, _T("comm send to acia data=%d pos:%d") ,recv_buff[recv_buff_r_pos],recv_buff_r_pos);
			...
			d_ctrl->write_signal(SIG_ACIA_RXCLK, 1, through ? 0xff : 1);
			// send data to acia
			d_ctrl->write_signal(SIG_ACIA_RXDATA, recv_buff[recv_buff_r_pos], through ? 0xff : 1);
			recv_buff_r_pos++;
...
//			d_ctrl->write_signal(SIG_ACIA_RXDATA, 1, 1);
		}

		// receive from acia
		if (is_rs) {
//			logging->out_debug("COMM: event_callback: %d", event_id);
			d_ctrl->write_signal(SIG_ACIA_TXCLK, 1, through ? 0xff : 1);
		}
	}
}
...
//		if (*size0 == 0) {
//			if (d_ctrl) d_ctrl->write_signal(SIG_ACIA_ERR_OVRN, 1, 1);
//		}
...
void COMM::load_state(FILEIO *fp)
{
