...
#define EVENT_MOTOR_TIMEOUT	2
#define EVENT_INDEXHOLE_ON	3
#define EVENT_INDEXHOLE_OFF	4

#define EVENT_READY_ON_0	5
#define EVENT_READY_ON_1	6
#define EVENT_READY_ON_2	7
#define EVENT_READY_ON_3	8
#define EVENT_READY_ON_4	9
#define EVENT_READY_ON_5	10
#define EVENT_READY_ON_6	11
#define EVENT_READY_ON_7	12
#define EVENT_MOTOR_OFF		13

// index hole (usually 300rpm -> 200ms)
#define DELAY_INDEX_HOLE	  200000
//#define DELAY_INDEX_HOLE	   50000
// index hole (8inch/5inch 2HD) (usually 360rpm -> 167ms)
#define DELAY_INDEX_HOLE_H	  166667

// ready on delay time after motor on (us)
#define DELAY_READY_ON		 1000000
// ready on delay time after motor on (us) (8inch/5inch 2HD) 
#define DELAY_READY_ON_H	  500000

// motor warmup time (us)
#define DELAY_MOTOR_WARMUP	  500000
// motor off delay time (us)
#define DELAY_MOTOR_OFF_3	 3000000
#define DELAY_MOTOR_OFF_5S	 5000000
#define DELAY_MOTOR_OFF_5	11000000
#define DELAY_MOTOR_TIMEOUT	60000000

#define DRIVE_MASK	(MAX_DRIVE - 1)

/// head loaded time 60ms
#define HEAD_LOADED_TIME	60000
#define HEAD_LOADED_CLOCK	(HEAD_LOADED_TIME * (CPU_CLOCKS / 1000000))

#define DELAY_WRITE_FRAME	300
...
void FLOPPY::initialize()
{
	...
	UTILITY::tcscpy(wav_file[2][0], sizeof(wav_file[2][0]) / sizeof(wav_file[2][0][0]), _T("fddseek5h.wav"));
	UTILITY::tcscpy(wav_file[2][1], sizeof(wav_file[2][1]) / sizeof(wav_file[2][1][0]), _T("fddmotor5h.wav"));
	UTILITY::tcscpy(wav_file[2][2], sizeof(wav_file[2][2]) / sizeof(wav_file[2][2][0]), _T("fddheadon5h.wav"));
	UTILITY::tcscpy(wav_file[2][3], sizeof(wav_file[2][3]) / sizeof(wav_file[2][3][0]), _T("fddheadoff5h.wav"));
...
void FLOPPY::warm_reset(bool por)
{
	...
	ff00_mask = (NOW_S1_MODE && config.fdd_type == FDD_TYPE_58FDD && FLG_ORIG_FDMSK == 0);
	...
	if (IOPORT_USE_FDD) {
		...
		d_fdc5->write_signal(SIG_MB8866_CLOCKNUM, config.fdd_type == FDD_TYPE_58FDD ? 1 : 0, 1);
	}
}
...
void FLOPPY::load_wav()
{
	...
	data_max[FLOPPY_WAV_HEADOOFF] = (sample_rate / 5);	// 0.2sec (head off)
	for (int ft=0; ft<FLOPPY_WAV_FDDTYPES; ft++) {
		for (int ty=0; ty<FLOPPY_WAV_SNDTYPES; ty++) {
		...
		for (int ft=0; ft<FLOPPY_WAV_FDDTYPES; ft++) {
			for (int ty=0; ty<FLOPPY_WAV_SNDTYPES; ty++) {
				...
				if ((wav_enable & (1 << j)) == 0) {
					...
					if(fre->Fopen(file_path, FILEIO_READ_BINARY)) {
...
void FLOPPY::write_io8(uint32_t addr, uint32_t data)
{
	if (IOPORT_USE_5FDD) {
		// 5inch
//		int ndrv_num = drv_num[0];

		if (config.fdd_type == FDD_TYPE_58FDD && NOW_L3_MODE) {
			addr = (addr & ~0x10) | (0x10 - (addr & 0x10));
		}
		...
		switch(addr & 0x1f) {
			...
			case 0x2:
				// FDC access ok
				if (config.fdd_type != FDD_TYPE_58FDD || (config.fdd_type == FDD_TYPE_58FDD && (drvsel[0] & 0x80) == 0 && !ff00_mask)) {
					OUT_DEBUG(_T("fddw a:%05x d:%02x (%02x)"),addr,data,(~data) & 0xff);
					d_fdc5->write_io8(addr & 0xf, data);
				} else {
			...
			case 0x3:
				...
				if (config.fdd_type != FDD_TYPE_58FDD || (config.fdd_type == FDD_TYPE_58FDD && ((drvsel[0] & fdd5outreg_delay & 0x80) == 0) && !ff00_mask)) {
					OUT_DEBUG(_T("fddw a:%05x d:%02x (%02x)"),addr,data,(~data) & 0xff);
					d_fdc5->write_io8(addr & 0xf, data);
				} else {
					OUT_DEBUG(_T("fddw a:%05x d:%02x (%02x) BLOCKED"),addr,data,(~data) & 0xff);
			...
			case 0x13:
				...
				if (config.fdd_type == FDD_TYPE_58FDD) {
					OUT_DEBUG(_T("fddw a:%05x d:%02x (%02x)"),addr,data,(~data) & 0xff);
					d_fdc5ex->write_io8(addr & 0xf, data);
				}
				break;
			case 0x14:
				set_drive_select(2, data);
				OUT_DEBUG(_T("fddw a:%05x d:%02x"),addr,data);
				break;
		}
...
//		int ndrv_num = drv_num[0];
		...
		switch(addr & 0x1f) {
			...
			case 0x13:
				// FDC access
				if (config.fdd_type == FDD_TYPE_58FDD) {
					data = d_fdc5ex->read_io8(addr & 0xf);
				}
				break;
			case 0x14:
				if (config.fdd_type == FDD_TYPE_58FDD) {
					data = (fdd5outreg[1] | 0x7e);
//					OUT_DEBUG(_T("fddr a:%05x d:%02x"),addr,data);
				}
				break;
		}
	} else if (IOPORT_USE_3FDD) {
...
void FLOPPY::write_signal(int id, uint32_t data, uint32_t mask)
{
	...
	switch (id) {
		...
		case SIG_FLOPPY_TRACK_SIZE:
			if (config.fdd_type == FDD_TYPE_58FDD && fdcnum == 0) {
				// 26sectors max
		...
		case SIG_CPU_RESET:
			...
			warm_reset(false);
			if (now_reset) {
				set_drive_select(IOPORT_USE_3FDD ? 0 : (config.fdd_type == FDD_TYPE_58FDD ? 2 : 1), 0);
//			} else {
//				if (config.fdd_type == FDD_TYPE_58FDD) d_fdc5->write_signal(id, data, mask);
			}
			break;
	}
...
//		}
		...
		case SIG_FLOPPY_DRQ:
			if (drqflg != drqflgprev) {
				if (config.fdd_type == FDD_TYPE_58FDD && fdcnum == 0) { // halt release
	...
	for(int i=0; i<MAX_DRIVE; i++) {
		if (fdd[i].head_loading & 0xff) {
			...
			if (disk[i]->inserted && fdd[i].head_loading == 0) {
				...
				wav_play[FLOPPY_WAV_HEADOOFF] = 1;
				wav_play_pos[FLOPPY_WAV_HEADOOFF] = 0;
			}
		}
		...
		case EVENT_READY_ON_7:
			{
				...
				if (ff00_mask && disk[i]->inserted && fdd[i].ready >= 2) {
					ff00_mask = false;
				}
			}
			break;
...
// ----------------------------------------------------------------------------
/// @param [in] type 0:3inch 1:5inch 2:5inch ex
...
//		d_fdc5->write_signal(SIG_MB8866_DRIVEREG, ndrv_num, DRIVE_MASK);
	} else {
		// 3inch
		switch(data & 0x0f) {
			case 2:
		...
		if (drv_num[fdcnum] != ndrv_num) {
			// update status
			d_fdc3->write_signal(SIG_MC6843_UPDATESTATUS, 0, 0);
		}
	}
...
//		d_fdc5->write_signal(SIG_MB8866_SIDEREG, data, 0x10);
		// density
		density = (data & 0x20) ? 1 : 0;
		d_fdc5->write_signal(SIG_FLOPPY_DENSITY, density, 1);

		// motor
		if ((data & 0x08) == 0 && motor_on_expand[fdcnum] != 0) {
			// motor on -> off
...
//		d_fdc5->write_signal(SIG_MB8866_MOTOR, drvsel, 0x08);
...
//		cancel_my_event(EVENT_MOTOR_OFF);
		...
		OUT_DEBUG(_T("fdd %d MOTOR OFF  SOUND:%d"), drv, wav_play[FLOPPY_WAV_MOTOR]);
	}
}
...
int FLOPPY::search_sector(int channel, int track, int sect, bool compare_side, int side)
{
	...
	int sta = search_sector_main(fdcnum, drvnum, sectorcnt);
	if (sta) {
		return sta;
	}
...
uint16_t FLOPPY::get_drive_select()
{
	...
	if (!config.now_power_off) {
		...
		case FDD_TYPE_58FDD:
			// 5inch 2hd
			for (int fdcnum = 0; fdcnum < 2; fdcnum++) {
				int i = drv_num[fdcnum];
				if (fdd[i].head_loading > 0 || ((drvsel[fdcnum] & 0x0b) == (0x08 | i))) {
					data |= ((1 << i) | (0x10 << i));
				}
			}
			break;
...
void FLOPPY::mix(int32_t* buffer, int cnt)
{
	...
	for(int typ=0; typ<FLOPPY_WAV_SNDTYPES; typ++) {
		if (wav_play[typ] && wav_size[fdd_type][typ]) {
			buffer = buffer_start;
			for(int i=0; i<cnt; i++) {
				*buffer++ += ((int)wav_data[fdd_type][typ][wav_play_pos[typ]] - 128) * wav_volume / 128;
				*buffer++ += ((int)wav_data[fdd_type][typ][wav_play_pos[typ]] - 128) * wav_volume / 128;
				wav_play_pos[typ]++;

				if (wav_play_pos[typ] >= wav_size[fdd_type][typ]) {
					wav_play_pos[typ] = 0;
					if (typ != FLOPPY_WAV_MOTOR) {
...
{
	wav_volume = int(16384.0 * pow(10.0, decibel / 40.0));
...
void FLOPPY::save_state(FILEIO *fp)
{
	...
	vm_state.drv_num2 = drv_num[1];
	vm_state.drvsel2  = drvsel[1];
	vm_state.fdd5outreg2 = fdd5outreg[1];
	vm_state.ff00_mask = ff00_mask ? 1 : 0;
...
void FLOPPY::load_state(FILEIO *fp)
{
	...
	if (Uint16_LE(vm_state_i.version) == 1) {
		...
		if (Uint16_LE(vm_state_i.version) >= 3) {
			drv_num[1] = vm_state.drv_num2;
			drvsel[1] = vm_state.drvsel2;
			fdd5outreg[1] = vm_state.fdd5outreg2;
		}
		if (Uint16_LE(vm_state_i.version) >= 0x43) {
			ff00_mask = vm_state.ff00_mask ? true : false;
		}
	}
	// shift event id
	...
	if ((Uint16_LE(vm_state_i.version) & 0xf) >= 5) {
		...
		switch(addr & 0x1f) {
			...
			case 0x1:
				...
				if (config.fdd_type != FDD_TYPE_58FDD || (config.fdd_type == FDD_TYPE_58FDD && (drvsel[0] & 0x80) == 0)) {
					data = d_fdc5->debug_read_io8(addr & 0xf);
				}
				break;
			case 0x2:
				// FDC access ok
			...
			case 0x13:
				// FDC access
				if (config.fdd_type == FDD_TYPE_58FDD) {
					data = d_fdc5ex->debug_read_io8(addr & 0xf);
				}
				break;
			case 0x14:
				if (config.fdd_type == FDD_TYPE_58FDD) {
...
void FLOPPY::debug_regs_info(_TCHAR *buffer, size_t buffer_len)
{
	...
	if (IOPORT_USE_5FDD) {
		...
		if (config.fdd_type == FDD_TYPE_58FDD) {
			...
			UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 0, _T("FDD5 2HD UNITSEL"), drvsel[idx]);
			UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 1, _T("FDD5 2D UNITSEL"), drvsel[1-idx]);
		} else {
