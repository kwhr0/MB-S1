...
void MEMORY::write_data8w(uint32_t addr, uint32_t data, int *wait)
{
	...
	if (addr < 0xf0000) {
		...
		// vram area
		if (NOW_L3_MODE && L3_ADDR_VRAM_START <= addr_64kb && addr_64kb < L3_ADDR_VRAM_END) {
#ifdef _DEBUG_CRAM
...
void MEMORY::reset_s1_l3_mode()
{
	...
	d_crtc->write_signal(SIG_HD46505_CHR_CLOCKS, CLOCKS_1MHZ * (NOW_S1_MODE || (REG_MODE_SEL & 0x80) ? 2 : 1), 0xffffffff);
	d_disp->write_signal(SIG_DISPLAY_CHR_CLOCKS, CLOCKS_1MHZ * (NOW_S1_MODE || (REG_MODE_SEL & 0x80) ? 2 : 1), 0xffffffff);
...
void MEMORY::load_state(FILEIO *fio)
{
...
//		pc_prev = vm_state_l3.pc_prev;
		fio->Fseek(sizeof(vm_state_l3->trace_counter_id), FILEIO_SEEK_CUR);
		fio->Fseek(sizeof(vm_state_l3->pc), FILEIO_SEEK_CUR);
		fio->Fseek(sizeof(vm_state_l3->pc_prev), FILEIO_SEEK_CUR);
		trace_counter_phase = TRACE_COUNTER_PHASE_IDLE;
		trace_count_remain = 0;
		prev_trace_clock = 0;
		...
		fio->Fseek(sizeof(vm_state_l3->reserved), FILEIO_SEEK_CUR);
...
//		pc_prev = vm_state_s1.pc_prev;
		if (vm_state_i.version >= 0x42) {
			...
			fio->Fseek(sizeof(vm_state_s1->trace_counter_phase), FILEIO_SEEK_CUR);
			fio->Fseek(sizeof(vm_state_s1->trace_count_remain), FILEIO_SEEK_CUR);
			fio->Fseek(sizeof(vm_state_s1->prev_trace_clock), FILEIO_SEEK_CUR);
			trace_counter_phase = TRACE_COUNTER_PHASE_IDLE;
			trace_count_remain = 0;
			prev_trace_clock = 0;
		}
		...
		fio->Fseek(sizeof(vm_state_s1->reserved1), FILEIO_SEEK_CUR);
		...
		if (Uint16_LE(vm_state_i.version) >= 0x43) {
			...
			fio->Fseek(sizeof(vm_state_s1->v43.reserved3), FILEIO_SEEK_CUR);
			fio->Fseek(sizeof(vm_state_s1->v43.exram_ident), FILEIO_SEEK_CUR);
			// load extended memory
			exram_size = fio->FgetUint16_LE();
		} else {
			// version ..0x42
			fio->Fread(addr_map[0], sizeof(addr_map[0]), 1);
			fio->Fseek(sizeof(vm_state_s1->v42.reserved2), FILEIO_SEEK_CUR);
			fio->Fseek(sizeof(vm_state_s1->v42.exram_ident), FILEIO_SEEK_CUR);
			// load extended memory
			exram_size = fio->FgetUint16_LE();
