...
- (id)init
{
	...
	hbox = [box_one addBox:HorizontalBox :MiddlePos :0 :_T("ClockSnd")];

	lbl = [CocoaLabel createI:CMsg::Clock_of_FM_Synthesis_Card_ASTERISK];
	[hbox addControl:lbl];
	[tab_view addSubview:lbl];

	popFmOpnClk = [CocoaPopUpButton createT:LABELS::fmopn_clock action:nil selidx:config.opn_clock];
	[hbox addControl:popFmOpnClk];
	[tab_view addSubview:popFmOpnClk];
	...
	lbl = [CocoaLabel createI:CMsg::Connect_interrupt_signal_of_FM_Synthesis_to_ASTERISK];
	[hbox addControl:lbl];
	[tab_view addSubview:lbl];
	...
	chkUseOpnExPsg = [CocoaCheckBox createI:CMsg::Use_FM_Synthesis_on_Extended_PSG_port action:nil value:config.use_opn_expsg];
	[box_one addControl:chkUseOpnExPsg];
	[tab_view addSubview:chkUseOpnExPsg];
...
//	config.wav_correct = ([chkCorrectWave state] == NSOnState);
	...
	config.show_dlg = ([chkUndefOp state] == NSOnState ? config.show_dlg | MSK_SHOWMSG_UNDEFOP : config.show_dlg & ~MSK_SHOWMSG_UNDEFOP);
	...
	config.use_opn_expsg = ([chkUseOpnExPsg state] == NSOnState);
#else
...
- (void)selectIO:(CocoaCheckBox *)sender
{
	...
	switch(num) {
		...
		case 5:
			// 9psg
			if ([sender state] == NSOnState) {
				[chkIOPort[6] setState:NSOffState];
#if defined(_MBS1)
				[chkIOPort[9] setState:NSOffState];
#endif
			}
			break;
		case 6:
			// Kanji
			if ([sender state] == NSOnState) {
				[chkIOPort[5] setState:NSOffState];
			}
			break;
#if defined(_MBS1)
		case 9:
			...
			if ([sender state] == NSOnState) {
				[chkIOPort[5] setState:NSOffState];
				[chkIOPort[6] setState:NSOnState];
			}
			break;
#endif
		default:
