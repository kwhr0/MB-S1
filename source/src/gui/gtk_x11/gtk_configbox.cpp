...
//	GtkWidget *btn;
	...
	hbox = create_hbox(vboxall);
	comFmOpnClk = create_combo_box(hbox, CMsg::Clock_of_FM_Synthesis_Card_ASTERISK, LABELS::fmopn_clock, config.opn_clock);
	create_label(hbox, "Hz");

	hbox = create_hbox(vboxall);
	comFmOpnIrq = create_combo_box(hbox, CMsg::Connect_interrupt_signal_of_FM_Synthesis_to_ASTERISK, LABELS::fmopn_irq, config.opn_irq);

	hbox = create_hbox(vboxall);
	chkUseOpnExPsg = create_check_box(hbox, CMsg::Use_FM_Synthesis_on_Extended_PSG_port, config.use_opn_expsg);
...
bool ConfigBox::SetData()
{
	...
	config.show_dlg = get_check_state(chkUndefOp) ? config.show_dlg | MSK_SHOWMSG_UNDEFOP : config.show_dlg & ~MSK_SHOWMSG_UNDEFOP;
	...
	config.use_opn_expsg = get_check_state(chkUseOpnExPsg);
