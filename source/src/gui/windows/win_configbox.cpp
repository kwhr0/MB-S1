...
INT_PTR ConfigBox::onInitDialog(UINT message, WPARAM wParam, LPARAM lParam)
{
...
#if defined(_MBS1)
	// MODE switch
	CBox *box_sysmode_sw = CreateGroup(box_0lv, IDC_STATIC_1, CMsg::System_Mode_ASTERISK, CBox::VerticalBox);
...
#else
	// DIP switch
	CBox *box_dip_sw = CreateGroup(box_0lv, IDC_STATIC_1, CMsg::DIP_Switch_ASTERISK, CBox::VerticalBox);
	...
	// FDD type select
	CBox *box_fdd_type = CreateGroup(box_0lv, IDC_STATIC_2, CMsg::FDD_Type_ASTERISK, CBox::VerticalBox);
	...
	CBox *box_ioport = CreateGroup(box_0rv, IDC_STATIC_3, CMsg::I_O_Port_Address_ASTERISK, CBox::VerticalBox);
	...
	// text
	CreateStatic(box_0all, IDC_STATIC_RESTART, CMsg::Need_restart_program_or_PowerOn);
	...
	// DirectX
	CBox *box_d3d = CreateGroup(box_1lv, IDC_STATIC_7, CMsg::Drawing, CBox::HorizontalBox);
...
//	(box_1rv);
	...
	CBox *box_crtc = CreateGroup(box_1rv, IDC_STATIC_6, CMsg::CRTC, CBox::HorizontalBox);
	...
	// text
	CreateStatic(box_1all, IDC_STATIC_RESTART_2, CMsg::Need_restart_program);
	...
	// Load wav file
	CBox *box_ldwav = CreateGroup(box_2lv, IDC_STATIC_4, CMsg::Load_Wav_File_from_Tape, CBox::VerticalBox);
	...
	CBox *box_svwav = CreateGroup(box_2rv, IDC_STATIC_5, CMsg::Save_Wav_File_to_Tape, CBox::HorizontalBox);
	...
	// FDD
	CBox *box_fdd = CreateGroup(box_2all, IDC_STATIC_8, CMsg::Floppy_Disk_Drive, CBox::VerticalBox);
...
//	int uart_w = font->GetTextWidth(hDlg, _T("mmmmmmmmmmmm"));
	...
	// text
	CreateStatic(box_4all, IDC_STATIC_RESTART_3, CMsg::Need_restart_program_or_PowerOn);
	...
	CBox *box_fmopn = box_5all->AddBox(CBox::HorizontalBox, 0, 0, _T("fmopn"));
	CreateStatic(box_fmopn, IDC_STATIC_FMOPN, CMsg::Clock_of_FM_Synthesis_Card_ASTERISK);
	CreateComboBox(box_fmopn, IDC_COMBO_FMOPN, LABELS::fmopn_clock, config.opn_clock, 5);
	CreateStatic(box_fmopn, IDC_STATIC_FMOPN_2, _T("Hz"));

	// interrupt
	CBox *box_fmint = box_5all->AddBox(CBox::HorizontalBox, 0, 0, _T("fmint"));
	CreateStatic(box_fmint, IDC_STATIC_FMOPN_IRQ, CMsg::Connect_interrupt_signal_of_FM_Synthesis_to_ASTERISK);
	CreateComboBox(box_fmint, IDC_COMBO_FMOPN_IRQ, LABELS::fmopn_irq, config.opn_irq, 5);

	// usg opn instead of psg
	CreateCheckBox(box_5all, IDC_CHK_USE_OPN_EXPSG, CMsg::Use_FM_Synthesis_on_Extended_PSG_port, config.use_opn_expsg);

	// text
	CreateStatic(box_5all, IDC_STATIC_RESTART_4, CMsg::Need_restart_program_or_PowerOn);
#endif
...
INT_PTR ConfigBox::onOK(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	// undef opcode
	config.show_dlg = (IsDlgButtonChecked(hDlg, IDC_CHK_UNDEFOP) == BST_CHECKED ? config.show_dlg | MSK_SHOWMSG_UNDEFOP : config.show_dlg & ~MSK_SHOWMSG_UNDEFOP);
