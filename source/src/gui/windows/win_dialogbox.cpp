...
HWND CDialogBox::CreateComboBox(CBox *box, int nItemId, const CMsg::Id *list, int selnum, int nMinSize, bool translate)
{
	DWORD dwStyle = WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | CBS_HASSTRINGS;
	HWND hCtrl = CreateWindow(WC_COMBOBOX, "", dwStyle , 0, 0, 10, 10, hDlg, (HMENU)(INT_PTR)nItemId, hInstance, NULL);
	if (hCtrl) {
...
void CDialogBox::SetComboBoxItems(int nItemId, const CMsg::Id *list, int selnum, bool translate)
{
	SetComboBoxItems(GetDlgItem(hDlg, nItemId), list, selnum, translate);
}
...
void CDialogBox::SetComboBoxItems(HWND combo, const CMsg::Id *list, int selnum, bool translate)
{
