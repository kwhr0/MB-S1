...
{
	config.show_dlg ^= MSK_USELIGHTPEN;
	if (FLG_USELIGHTPEN) {
...
{
	emu->toggle_mouse();
	config.show_dlg = (emu->get_mouse_enabled() ? (config.show_dlg | MSK_USEMOUSE) : (config.show_dlg & ~MSK_USEMOUSE));
	if (FLG_USEMOUSE) {
...
// -------------------------------------
#elif defined(USE_SDL) || defined(USE_SDL2)
void GUI_BASE::CreateLedBox(const _TCHAR *res_path, CPixelFormat *src_format)
{
#ifdef USE_LEDBOX
	ledbox = new LedBox();
