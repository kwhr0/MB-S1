...
void AG_CONFIG_DLG::Create()
{
	...
	param.use_opn_expsg = config.use_opn_expsg ? 1 : 0;
...
//	}
	...
	hbox = AG_BoxNewHoriz(vbox_base, AG_BOX_HFILL);
	AG_LabelNewS(hbox, 0, CMSG(Clock_of_FM_Synthesis_Card_ASTERISK));
	UComboNew(hbox, LABELS::fmopn_clock, paramtmp->fmopn_clock_num, OnChangeFmOpnClock);
	AG_LabelNewS(hbox, 0, _T("Hz"));
	//
	hbox = AG_BoxNewHoriz(vbox_base, AG_BOX_HFILL);
	AG_LabelNewS(hbox, 0, CMSG(Connect_interrupt_signal_of_FM_Synthesis_to_ASTERISK));
	UComboNew(hbox, LABELS::fmopn_irq, paramtmp->fmopn_irq, OnChangeFmOpnIrq);
	//
	AG_CheckboxNewInt(vbox_base, 0, CMSG(Use_FM_Synthesis_on_Extended_PSG_port), &paramtmp->use_opn_expsg);
...
int AG_CONFIG_DLG::SetData(AG_Window *win)
{
	...
	config.show_dlg = (param.showmsg_undefop ? config.show_dlg | MSK_SHOWMSG_UNDEFOP : config.show_dlg & ~MSK_SHOWMSG_UNDEFOP);
	...
	config.use_opn_expsg = (param.use_opn_expsg == 1);
#if defined(USE_Z80B_CARD)
